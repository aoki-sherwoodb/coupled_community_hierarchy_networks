...
class SequentialHierarchyCommunitySimple(NetworkEmbeddingModel):
    ...
    def log_likelihood(self, X):
        ...
        eps = 1e-9
        L = np.sum(self.adj_matrix * np.log(P + eps) + (1 - self.adj_matrix) * np.log(1 - P + eps))
        # for i in range(self.num_nodes):
        #     for j in range(self.num_nodes):
        #         if i == j:
        #             continue
...
class SequentialHierarchyCommunitySimpleJAX(NetworkEmbeddingModel):
    def __init__(self, adj_matrix, embedding_dim, alpha=5, beta=1):
        ...
        self.embeddings = jax.random.normal(jax.random.PRNGKey(0), (self.num_nodes, self.embedding_dim))
    ...
    def log_likelihood(self, X):
        ...
        L = jnp.sum(
            self.adj_matrix * jnp.log(P + eps) + (1 - self.adj_matrix) * jnp.log(1 - P + eps)
        )
    ...
    def predict(self, i, j):
        ...
        direction_prob = 1 / (1 + jnp.exp(-self.beta * (1 + cos_sim_matrix[i, j]) * (s[i] - s[j])))
    ...
    def generate(self, expected_num_edges=None):
        ...
        generated_adj = np.array(np.random.rand(self.num_nodes, self.num_nodes) < c * np.array(P)).astype(int)
...
class SequentialHierarchyCommunityMulti(NetworkEmbeddingModel):
    ...
    def log_likelihood(self, X):
        ...
        L_existence = np.sum(
            A_bar * (self.alpha * (cos_sim_matrix - 1) - np.log(Z))
        ) - m
    ...
    def predict_edge_count(self, i, j, c=None):
        ...
        edge_count = np.random.poisson(c_hat * np.exp(self.alpha * (cos_sim_matrix[i, j] - 1)))
        return edge_count
...
class SequentialHierarchyCommunityMultiJAX(NetworkEmbeddingModel):
    ...
    def log_likelihood(self, X):
        ...
        L_existence = jnp.sum(
            A_bar * (self.alpha * (cos_sim_matrix - 1) - jnp.log(Z))
        ) - m
